...
from GB.GranularBallUCI import getGranularBall
...
def main():
    keys = ['abalone','PenDigits','drybean','Waveform']
    ress = [3,10,7,3]
    detas = [0.1,0.1,0.1,0.1]
    ...
    for d in range(len(keys)):
        ...
        time1 = time.time()
        target,data,gb_list,data_num = getGranularBall(keys[d])
